Shader "Bloodsport/WorldAligned_Skinned"
{
    Properties
    {
        ...
        [HideInInspector] _ObjectRotRow0 ("Object Rot Row 0", Vector) = (1,0,0,0)
        [HideInInspector] _ObjectRotRow1 ("Object Rot Row 1", Vector) = (0,1,0,0)
        [HideInInspector] _ObjectRotRow2 ("Object Rot Row 2", Vector) = (0,0,1,0)
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                float4 _ObjectRotRow0;
                float4 _ObjectRotRow1;
                float4 _ObjectRotRow2;
            CBUFFER_END
            ...
            float4 frag(Varyings IN) : SV_Target
            {
                ...
                // UNITY_MATRIX_M is not reliable for SkinnedMeshRenderers - the
                // skinning pipeline transforms vertices via the root bone before
                // the standard model matrix is applied, so it does not track the
                // gun's real transform during animation. Origin and rotation are
                // instead fed in from script every frame via MaterialPropertyBlock
                // (see SkinnedWorldAlignedOrigin.cs), sourced from the real Transform.
                float3 relativeWS   = IN.positionWS - _ObjectOriginWS.xyz;
                float3x3 modelRot   = float3x3(_ObjectRotRow0.xyz, _ObjectRotRow1.xyz, _ObjectRotRow2.xyz);
