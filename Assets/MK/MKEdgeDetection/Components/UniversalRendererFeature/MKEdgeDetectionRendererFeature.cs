...
/* Version: 1.2.8  */
/* Exported on: 21.03.2026 14:58:28 */
		...
		public sealed class MKEdgeDetectionRenderPass : UnityEngine.Rendering.Universal.ScriptableRenderPass, MK.EdgeDetection.PostProcessing.Generic.ICameraData
		{
			...
			*/
			...
			public override void Execute(UnityEngine.Rendering.ScriptableRenderContext context, ref UnityEngine.Rendering.Universal.RenderingData renderingData)
			{
				...
				{
					_passData.width = _passData.widthRaw = Mathf.RoundToInt(renderingData.cameraData.cameraTargetDescriptor.width * UnityEngine.ScalableBufferManager.widthScaleFactor);
					_passData.height = _passData.heightRaw = Mathf.RoundToInt(renderingData.cameraData.cameraTargetDescriptor.height * UnityEngine.ScalableBufferManager.heightScaleFactor);
				}
				else
				{
					_passData.width = _passData.widthRaw = renderingData.cameraData.cameraTargetDescriptor.width;
					_passData.height = _passData.heightRaw = renderingData.cameraData.cameraTargetDescriptor.height;
				}
			...
			private void DrawEffect(UnityEngine.Rendering.RenderTargetIdentifier source, UnityEngine.Rendering.RenderTargetIdentifier destination, MK.EdgeDetection.UniversalRendererFeatures.IParameters properties, MK.EdgeDetection.PostProcessing.Generic.ICameraData cameraData, UnityEngine.Rendering.CommandBuffer cmd)
			{
				...
				Vector2Int screenSize = cameraData.isSceneView ? new Vector2Int(cameraData.width, cameraData.height) : new Vector2Int(cameraData.widthRaw, cameraData.heightRaw);
				...
				_materialPropertyBlock.SetVector(PropertyIDs.mkShaderMainTexDimension, new Vector2(screenSize.x, screenSize.y));
				...
				float logDepthSize = AdjustSize(properties.getDepthLineSize * properties.getGlobalLineSize * pixelPerfectionScale * sketchLinesScale, screenSize.x, screenSize.y, properties.getLineSizeMatchFactor, cameraData.allowDynamicResolution);
				float logNormalLineSize = AdjustSize(properties.getNormalLineSize * properties.getGlobalLineSize * pixelPerfectionScale * sketchLinesScale, screenSize.x, screenSize.y, properties.getLineSizeMatchFactor, cameraData.allowDynamicResolution);
				float logSceneColorLineSize = AdjustSize(properties.getSceneColorLineSize * properties.getGlobalLineSize * pixelPerfectionScale * sketchLinesScale, screenSize.x, screenSize.y, properties.getLineSizeMatchFactor, cameraData.allowDynamicResolution);
				...
				{
					MKBlitter.GetTemporaryTexture(cmd, PropertyIDs.edgeMask, MKBlitter.GetRenderTextureDescriptor(cameraData, screenSize.x, screenSize.y, RenderTextureFormat.Default, false));
					
					MKBlitter.Draw(cmd, PropertyIDs.edgeMask, renderMaterial, _materialPropertyBlock, 0, new Rect(0, 0, screenSize.x, screenSize.y));
					...
					MKBlitter.Draw(cmd, destination, sketchMaterial, _materialPropertyBlock, 0, new Rect(0, 0, screenSize.x, screenSize.y));
					
					MKBlitter.ReleaseTemporaryTexture(cmd, PropertyIDs.edgeMask);
				}
				else
				{
					MKBlitter.Draw(cmd, destination, renderMaterial, _materialPropertyBlock, 0, new Rect(0, 0, screenSize.x, screenSize.y));
				}
			}
			...
			public override void RecordRenderGraph(UnityEngine.Rendering.RenderGraphModule.RenderGraph renderGraph, UnityEngine.Rendering.ContextContainer frameData)
			{
				using (var builder = renderGraph.AddUnsafePass<PassData>("MKEdgeDetection", out var passData))
				{
					...
					{
						passData.width = passData.widthRaw = Mathf.RoundToInt(universalCameraData.cameraTargetDescriptor.width * UnityEngine.ScalableBufferManager.widthScaleFactor);
						passData.height = passData.heightRaw = Mathf.RoundToInt(universalCameraData.cameraTargetDescriptor.height * UnityEngine.ScalableBufferManager.heightScaleFactor);
					}
					else
					{
						passData.width = passData.widthRaw = universalCameraData.cameraTargetDescriptor.width;
						passData.height = passData.heightRaw = universalCameraData.cameraTargetDescriptor.height;
					}
			...
			public int widthRaw => _passData.widthRaw;
			public int heightRaw => _passData.heightRaw;
