Shader "Bloodsport/WorldAligned_Fixed"
{
    Properties
    {
        ...
        _AnchorPosWS     ("Anchor Pos WS",       Vector)     = (0,0,0,0)
        _AnchorR0        ("Anchor Rot Row0",      Vector)     = (1,0,0,0)
        _AnchorR1        ("Anchor Rot Row1",      Vector)     = (0,1,0,0)
        _AnchorR2        ("Anchor Rot Row2",      Vector)     = (0,0,1,0)
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings IN) : SV_Target
            {
                ...
                // Use script-provided root transform so skinning deformation
                // on individual vertices does not affect the sample position.
                float3 rel = IN.positionWS - _AnchorPosWS.xyz;
                float3x3 anchorRot = float3x3(_AnchorR0.xyz, _AnchorR1.xyz, _AnchorR2.xyz);
                float3 posForSample = mul(anchorRot, rel);
