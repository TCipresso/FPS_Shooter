    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                // solid backdrop across the whole pad shape, rings glow on top of it
                half4 col = _BackgroundColor;
                col.rgb += ringColor.rgb * ringLine * _GlowIntensity;
                col.rgb = lerp(col.rgb, _OutlineColor.rgb, outlineMask);
                float coverage = saturate(fillMask * max(_BackgroundColor.a, ringLine) + outlineMask);
