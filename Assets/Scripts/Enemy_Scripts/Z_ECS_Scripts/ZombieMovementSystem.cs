...
public partial struct ZombieMovementSystem : ISystem
{
    public void OnUpdate(ref SystemState state)
    {
        ...
        float separationStrength = 2f;
        float standFootprintRadius = 0.6f;
        float climbHeightThreshold = 0.3f;
        float forwardDotThreshold = 0.95f;
        float standTolerance = 0.3f;
        if (SystemAPI.TryGetSingleton<ZombieWallConfig>(out ZombieWallConfig wallConfig))
        {
            ...
            separationRadius = wallConfig.SeparationRadius;
            separationStrength = wallConfig.SeparationStrength;
            standFootprintRadius = wallConfig.StandFootprintRadius;
            climbHeightThreshold = wallConfig.ClimbHeightThreshold;
            forwardDotThreshold = wallConfig.ForwardDotThreshold;
            standTolerance = wallConfig.StandTolerance;
        }
        ...
        JobHandle desiredMoveHandle = new ZombieDesiredMoveJob
        {
            ...
            StandFootprintRadius = standFootprintRadius,
            ClimbHeightThreshold = climbHeightThreshold,
            ForwardDotThreshold = forwardDotThreshold,
            StandTolerance = standTolerance,
...
partial struct ZombieDesiredMoveJob : IJobEntity
{
    ...
    public float StandFootprintRadius;
    public float ClimbHeightThreshold;
    public float ForwardDotThreshold;
    public float StandTolerance;
...
partial struct ZombieApplyMovementJob : IJobEntity
{
    ...
    void Execute([EntityIndexInQuery] int index, ref LocalTransform transform, in ZombieMoveSpeed moveSpeed, ref ZombieContactCooldown cooldown, ref ZombieVerticalVelocity verticalVelocity, ref ZombieClimbState climbState, in ZombieGroundOffset groundOffset, in ZombieTarget target, in ZombieContactDamage contactDamage, ref ZombieEmerge emerge)
    {
        ...
        if (target.HasTarget && distToPlayer < ContactRadius)
        {
            ...
            {
                PlayerDamageWriter.Enqueue(new PlayerDamageEvent { PlayerIndex = target.Index, Amount = contactDamage.Value });
