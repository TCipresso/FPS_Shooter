...
public abstract class ZombieBase : MonoBehaviour
{
    ...
    Dictionary<WeaponBase, int> weaponDamageContributors = new Dictionary<WeaponBase, int>();
    ...
    public virtual void TakeDamage(int amount, PlayerStats dealer, float weaponMultiplier = 1f, Vector3 hitDirection = default, float ragdollForceMultiplier = 1f, string hitBone = "", WeaponBase weapon = null)
    {
        if (isDead) return;
        ...
        if (weapon != null)
        {
            if (weaponDamageContributors.TryGetValue(weapon, out int wExisting))
                weaponDamageContributors[weapon] = wExisting + actualDamage;
            else
                weaponDamageContributors[weapon] = actualDamage;
        }
    ...
    public virtual void ResetEnemy()
    {
        ...
        weaponDamageContributors.Clear();
        totalDamageDealt = 0;
        lastAttackTime = 0f;
    ...
    void HandleDeath()
    {
        ...
        foreach (var kvp in weaponDamageContributors)
        {
            WeaponBase weapon = kvp.Key;
