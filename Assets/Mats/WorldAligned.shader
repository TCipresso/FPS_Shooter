Shader "Bloodsport/WorldAligned"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 TriplanarSample(TEXTURE2D_PARAM(tex, samp), float3 posWS, float3 weights, float scale)
            {
                float4 xProj = SAMPLE_TEXTURE2D(tex, samp, posWS.zy * scale);
                float4 yProj = SAMPLE_TEXTURE2D(tex, samp, posWS.xz * scale);
                float4 zProj = SAMPLE_TEXTURE2D(tex, samp, posWS.xy * scale);
            ...
            float4 frag(Varyings IN) : SV_Target
            {
                ...
                float3 weights    = TriplanarWeights(IN.normalWS);
                float4 shapeSample = TriplanarSample(
                    TEXTURE2D_ARGS(_ShapeTex, sampler_ShapeTex),
                    IN.positionWS, weights, 1.0 / _ShapeScale
                );
